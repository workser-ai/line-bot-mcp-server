...
export function registerSetRichMenuDefault(server: FastMCP<LineSessionData>) {
  server.addTool({
    ...
    execute: async (args, context) => {
      try {
        ...
        const response = await client.setDefaultRichMenu(args.richMenuId);
        return createSuccessResponse(response);
      } catch (error: unknown) {
        const message =
          error instanceof Error ? error.message : "Unknown error";
        return createErrorResponse(
          `Failed to set default rich menu: ${message}`,
        );
      }
    },
  });
